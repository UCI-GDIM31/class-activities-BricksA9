...
public class DeerW5 : MonoBehaviour
{
    ...
    NavMeshAgent followTargetNavMesh;

    void Start()
    {
        followTargetNavMesh = GetComponent<NavMeshAgent>();
    }

    void Update()
    {
        //grab location of target gameObject
        Vector3 targetObjectLocation = followTarget.position;

        followTargetNavMesh.SetDestination(targetObjectLocation);
    }
}